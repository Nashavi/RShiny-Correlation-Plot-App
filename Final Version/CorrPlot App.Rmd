...
#print("Plot of AS Item No: ",input$asitmno)
#print(cordf[cordf$ASItemNo=input$asitmno,])

g0<-ggplot() + 
  geom_line(data = tmpdf, aes(y = stdcom, x = MonthDate, color = "Commodity Price")) +geom_line(data = tmpdf, aes(y = stitm, x = MonthDate, color = "Item Price",text = paste("R2 value:",round(cor0*100,2),"%"))) + ylab('Price') + xlab('Month Date')+ggtitle("IP vs CP")+theme(axis.text.x=element_text(angle=50, vjust=0.5))+theme_dark()
